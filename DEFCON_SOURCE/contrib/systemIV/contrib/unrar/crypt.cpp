...
#define substLong(t) ( (unsigned int)SubstTable[(unsigned int)t&255] | \
           ((unsigned int)SubstTable[(int)(t>> 8)&255]<< 8) | \
           ((unsigned int)SubstTable[(int)(t>>16)&255]<<16) | \
           ((unsigned int)SubstTable[(int)(t>>24)&255]<<24) )
...
#ifndef SFX_MODULE
static unsigned char InitSubstTable[256]={
...
void CryptData::DecryptBlock(unsigned char *Buf,int Size)
{
...
#ifndef SFX_MODULE
void CryptData::EncryptBlock20(unsigned char *Buf)
{
...
void CryptData::UpdKeys(unsigned char *Buf)
{
...
void CryptData::Swap(unsigned char *Ch1,unsigned char *Ch2)
{
...
{
  unsigned int PswCRC=CRC(0xffffffff,Password,strlen(Password));
...
void CryptData::Crypt(unsigned char *Data,unsigned int Count,int Method)
{
...
void CryptData::Encode13(unsigned char *Data,unsigned int Count)
{
  while (Count--)
  {
    PN2+=PN3;
...
void CryptData::Decode13(unsigned char *Data,unsigned int Count)
{
  while (Count--)
  {
    PN2+=PN3;
...
void CryptData::Crypt15(unsigned char *Data,unsigned int Count)
{
  while (Count--)
  {
    OldKey[0]+=0x1234;
