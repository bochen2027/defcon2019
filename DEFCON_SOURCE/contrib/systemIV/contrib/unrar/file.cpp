...
bool File::Open(const char *Name,const wchar *NameW,bool OpenShared,bool Update)
{
  ...
  unsigned int ShareMode=FILE_SHARE_READ;
  if (OpenShared)
  ...
  while (1)
  {
    ...
    if (HandleType!=FILE_HANDLENORMAL)
    {
      ...
      for (int I=0;I<Size;I+=MaxSize)
        if (!(Success=WriteFile(hFile,(unsigned char *)Data+I,Min(Size-I,MaxSize),&Written,NULL)))
          break;
    }
    else
...
#ifdef _WIN_32
  LONG HighDist=0;
  unsigned int LowDist=SetFilePointer(hFile,0,&HighDist,FILE_CURRENT);
...
void File::SetCloseFileStat(RarTime *ftm,RarTime *fta,unsigned int FileAttr)
{
#ifdef _WIN_32
...
bool File::IsDevice()
{
  if (hFile==BAD_HANDLE)
    return(false);
#ifdef _WIN_32
  unsigned int Type=GetFileType(hFile);
