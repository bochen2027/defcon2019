...
void ModelPPM::RestartModelRare()
{
  ...
  static const unsigned short InitBinEsc[]={
...
NO_LOOP:
  ...
  if (pc->NumStats != 1) 
  {
    if ((unsigned char*) pc <= SubAlloc.pText)
      return(NULL);
    ...
    unsigned int s0=pc->U.SummFreq-pc->NumStats-cf;
...
inline void ModelPPM::UpdateModel()
{
  ...
  *SubAlloc.pText++ = fs.Symbol;                   
  ...
  if ( fs.Successor ) 
  {
    if ((unsigned char*) fs.Successor <= SubAlloc.pText &&
...
inline void PPM_CONTEXT::decodeBinSymbol(ModelPPM *Model)
{
  ...
  unsigned short& bs=Model->BinSumm[rs.Freq-1][Model->PrevSuccess+
...
{
  if ((unsigned char*)MinContext <= SubAlloc.pText || (unsigned char*)MinContext>SubAlloc.HeapEnd)
    return(-1);
  ...
  if (!OrderFall && (unsigned char*) FoundState->Successor > SubAlloc.pText)
