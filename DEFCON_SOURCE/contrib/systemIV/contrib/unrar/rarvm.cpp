...
{
  if (Mem==NULL)
    Mem=new unsigned char[VM_MEMSIZE+4];
}
...
#if defined(BIG_ENDIAN) || !defined(ALLOW_NOT_ALIGNED_INT)
  #define GET_VALUE(ByteMode,Addr) GetValue(ByteMode,(unsigned int *)Addr)
#else
  #define GET_VALUE(ByteMode,Addr) ((ByteMode) ? (*(unsigned char *)(Addr)):UINT32(*(unsigned int *)(Addr)))
#endif


inline void RarVM::SetValue(bool ByteMode,unsigned int *Addr,unsigned int Value)
{
  if (ByteMode)
...
#if defined(BIG_ENDIAN) || !defined(ALLOW_NOT_ALIGNED_INT) || !defined(PRESENT_INT32)
  #define SET_VALUE(ByteMode,Addr,Value) SetValue(ByteMode,(unsigned int *)Addr,Value)
#else
  #define SET_VALUE(ByteMode,Addr,Value) ((ByteMode) ? (*(unsigned char *)(Addr)=(Value)):(*(uint32 *)(Addr)=((uint32)(Value))))
#endif


void RarVM::SetValue(unsigned int *Addr,unsigned int Value)
{
  SetValue(false,Addr,Value);
}


inline unsigned int* RarVM::GetOperand(VM_PreparedOperand *CmdOp)
{
  if (CmdOp->Type==VM_OPREGMEM)
    return((unsigned int *)&Mem[(*CmdOp->Addr+CmdOp->Base)&VM_MEMMASK]);
  else
    return(CmdOp->Addr);
}
...
void RarVM::PrintState(unsigned int IP)
{
...
void RarVM::Prepare(unsigned char *Code,int CodeSize,VM_PreparedProgram *Prg)
{
  InitBitInput();
...
#endif  
    unsigned int DataFlag=fgetbits();
    faddbits(1);
    if (DataFlag&0x8000)
    {
...
unsigned int RarVM::ReadData(BitInput &Inp)
{
  unsigned int Data=Inp.fgetbits();
  switch(Data&0xc000)
  {
    case 0:
      Inp.faddbits(6);
...
void RarVM::SetMemory(unsigned int Pos,unsigned char *Data,unsigned int DataSize)
{
...
#ifdef VM_STANDARDFILTERS
VM_StandardFilters RarVM::IsStandardFilter(unsigned char *Code,int CodeSize)
{
  ...
  unsigned int CodeCRC=CRC(0xffffffff,Code,CodeSize)^0xffffffff;
    ...
    case VMSF_E8E9:
      {
        ...
        int DataSize=R[4];
        unsigned int FileOffset=R[6];

        if (DataSize>=VM_GLOBALMEMADDR)
          break;

        const int FileSize=0x1000000;
        unsigned char CmpByte2=FilterType==VMSF_E8E9 ? 0xe9:0xe8;
        for (unsigned int CurPos=0;CurPos<DataSize-4;)
        {
          unsigned char CurByte=*(Data++);
          CurPos++;
    ...
    case VMSF_ITANIUM:
      {
        ...
        while (CurPos<DataSize-21)
        {
          ...
          if (Byte>=0)
          {
            ...
            unsigned char CmdMask=Masks[Byte];
            if (CmdMask!=0)
              for (int I=0;I<=2;I++)
                if (CmdMask & (1<<I))
                {
    ...
    case VMSF_RGB:
      {
        ...
        unsigned char *SrcData=Mem,*DestData=SrcData+DataSize;
        ...
        for (int CurChannel=0;CurChannel<Channels;CurChannel++)
        {
          ...
          for (int I=CurChannel;I<DataSize;I+=Channels)
          {
            ...
            if (UpperPos>=3)
            {
              unsigned char *UpperData=DestData+UpperPos;
              ...
              Predicted=PrevByte;
            DestData[I]=PrevByte=(unsigned char)(Predicted-*(SrcData++));
          }
        }
        for (int I=PosR,Border=DataSize-2;I<Border;I+=3)
        {
          unsigned char G=DestData[I+1];
          DestData[I]+=G;
          DestData[I+2]+=G;
        }
      }
    ...
    case VMSF_UPCASE:
      {
        ...
        {
          unsigned char CurByte=Mem[SrcPos++];
...
unsigned int RarVM::FilterItanium_GetBits(unsigned char *Data,int BitPos,int BitCount)
{
  int InAddr=BitPos/8;
  int InBit=BitPos&7;
  unsigned int BitField=(unsigned int)Data[InAddr++];
  BitField|=(unsigned int)Data[InAddr++] << 8;
  BitField|=(unsigned int)Data[InAddr++] << 16;
  BitField|=(unsigned int)Data[InAddr] << 24;
  BitField >>= InBit;
  return(BitField & (0xffffffff>>(32-BitCount)));
}


void RarVM::FilterItanium_SetBits(unsigned char *Data,unsigned int BitField,int BitPos,
                                  int BitCount)
{
  int InAddr=BitPos/8;
  int InBit=BitPos&7;
