...
void RawRead::Read(unsigned char *SrcData,int Size)
{
  if (Size!=0)
  {
    Data.Add(Size);
...
void RawRead::Get(unsigned char &Field)
{
  Field=Data[ReadPos];
  ReadPos++;
}
...
void RawRead::Get(unsigned short &Field)
{
...
void RawRead::Get(unsigned int &Field)
{
...
void RawRead::Get(unsigned char *Field,int Size)
{
...
unsigned int RawRead::GetCRC(bool ProcessedOnly)
{
