...
void ComprDataIO::UnpWrite(unsigned char *Addr,unsigned int Count)
{
#ifdef RARDLL
  ...
  if (!SkipUnpCRC)
    ...
    if (((Archive *)SrcFile)->OldFormat)
      UnpFileCRC=OldCRC((unsigned short)UnpFileCRC,Addr,Count);
    else
#endif
...
void ComprDataIO::GetUnpackedData(unsigned char **Data,unsigned int *Size)
{
  *Data=UnpWrAddr;
  *Size=UnpWrSize;
}
...
void ComprDataIO::SetEncryption(int Method,char *Password,unsigned char *Salt,bool Encrypt)
{
  if (Encrypt)
  {
...
void ComprDataIO::SetUnpackToMemory(unsigned char *Addr,unsigned int Size)
{
  UnpackToMemory=true;
