...
static unsigned char S[256],S5[256],rcon[30];
static unsigned char T1[256][4],T2[256][4],T3[256][4],T4[256][4];
static unsigned char T5[256][4],T6[256][4],T7[256][4],T8[256][4];
static unsigned char U1[256][4],U2[256][4],U3[256][4],U4[256][4];


inline void Xor128(unsigned char *dest,const unsigned char *arg1,const unsigned char *arg2)
{
...
inline void Xor128(unsigned char *dest,const unsigned char *arg1,const unsigned char *arg2,
                   const unsigned char *arg3,const unsigned char *arg4)
{
#if defined(PRESENT_INT32) && defined(ALLOW_NOT_ALIGNED_INT)
...
inline void Copy128(unsigned char *dest,const unsigned char *src)
{
#if defined(PRESENT_INT32) && defined(ALLOW_NOT_ALIGNED_INT)
...
void Rijndael::init(Direction dir,const unsigned char * key,unsigned char * initVector)
{
  m_direction = dir;
...
int Rijndael::blockDecrypt(const unsigned char *input, int inputLen, unsigned char *outBuffer)
{
  ...
  {
    decrypt(input, block);
    Xor128(block,block,(unsigned char*)iv);
#if STRICT_ALIGN
    memcpy(iv, input, 16);
    memcpy(outBuf, block, 16);
#else
    Copy128((unsigned char*)iv,input);
...
void Rijndael::keyEncToDec()
{
  ...
  {
    unsigned char n_expandedKey[4][4];
    for (int i=0;i<4;i++)
      for (int j=0;j<4;j++)
      {
        unsigned char *w=m_expandedKey[r][j];
...
void Rijndael::decrypt(const unsigned char a[16], unsigned char b[16])
{
  ...
  Xor128((unsigned char*)temp,(unsigned char*)a,(unsigned char*)m_expandedKey[m_uRounds]);
  ...
  {
    Xor128((unsigned char*)temp,(unsigned char*)b,(unsigned char*)m_expandedKey[r]);
    Xor128(b,   T5[temp[0][0]],T6[temp[3][1]],T7[temp[2][2]],T8[temp[1][3]]);
    Xor128(b+4, T5[temp[1][0]],T6[temp[0][1]],T7[temp[3][2]],T8[temp[2][3]]);
    Xor128(b+8, T5[temp[2][0]],T6[temp[1][1]],T7[temp[0][2]],T8[temp[3][3]]);
    Xor128(b+12,T5[temp[3][0]],T6[temp[2][1]],T7[temp[1][2]],T8[temp[0][3]]);
  ...
  Xor128((unsigned char*)temp,(unsigned char*)b,(unsigned char*)m_expandedKey[1]);
  b[ 0] = S5[temp[0][0]];
  b[ 1] = S5[temp[3][1]];
  b[ 2] = S5[temp[2][2]];
  b[ 3] = S5[temp[1][3]];
  ...
  b[14] = S5[temp[1][2]];
  b[15] = S5[temp[0][3]];
  Xor128((unsigned char*)b,(unsigned char*)b,(unsigned char*)m_expandedKey[0]);
}
...
#define fwd_affine(x) \
    (w = (unsigned int)x, w ^= (w<<1)^(w<<2)^(w<<3)^(w<<4), (unsigned char)(0x63^(w^(w>>8))))

#define inv_affine(x) \
    (w = (unsigned int)x, w = (w<<1)^(w<<3)^(w<<6), (unsigned char)(0x05^(w^(w>>8))))
...
void Rijndael::GenerateTables()
{
  ...
  for(int i = 0; i < 256; ++i)
  {   
    unsigned char b=S[i]=fwd_affine(FFinv((unsigned char)i));
    ...
    S5[i] = b = FFinv(inv_affine((unsigned char)i));
