...
void InterfaceWindow::CreateValueControl( char *name, int x, int y, int w, int h,
                                          ...
                                          bool _inputNoExtendedCharacters )
{
    ...
    if( dataType == InputField::TypeString )
    {
        ...
        input->SetInputNoExtendedCharacters( _inputNoExtendedCharacters );
    }
    else
    {
...
void TextButton::Render( int realX, int realY, bool highlighted, bool clicked )
{
	if( m_renderLanguage )
	{
		g_renderer->SetFont( NULL, m_renderLanguage );
	}
...
void TextButton::SetRenderLanguage( const char *_renderLanguage )
{
	if( m_renderLanguage )
	{
		delete [] m_renderLanguage;
