...
DropDownOptionData::DropDownOptionData(const char *_word, int _value, bool _wordIsLanguagePhrase,
                                       const char *_renderLanguage)
:   m_word(strdup(_word)), 
    m_value(_value),
    m_wordIsLanguagePhrase(_wordIsLanguagePhrase),
    m_renderLanguage(NULL)
{
	if( _renderLanguage )
	{
		m_renderLanguage = strdup( _renderLanguage );
	}
}
...
void DropDownMenu::AddOption( char const *_word, int _value, bool _optionIsLanguagePhrase,
                              const char *_renderLanguage )
{
	if (_value == INT_MIN)
	{
	...
	DropDownOptionData *option = new DropDownOptionData( _word, _value, _optionIsLanguagePhrase, _renderLanguage );
...
void DropDownMenu::SelectOption( int _value )
{
    ...
    if( m_currentOption < 0 || m_currentOption >= m_options.Size() )
    {
        if( m_orgcaption )
        {
            SetCaption( m_orgcaption, m_orgcaptionIsLanguagePhrase );
        }
        else
        {
    ...
    {
        DropDownOptionData *option = m_options[m_currentOption];
        SetCaption( option->m_word, option->m_wordIsLanguagePhrase );
    }
...
void DropDownMenu::CreateMenu()
{
    ...
    for( int col = 0; col < numColumnsRequired; ++col )
    {
        for( int i = 0; i < numPerColumn; ++i )
        {
            ...
            DropDownOptionData *option = m_options[index];
            char thisName[64];
            ...
            menuOption->SetProperties( thisName, col*m_w+2, titleH + i * m_h, w, m_h, option->m_word, " ", option->m_wordIsLanguagePhrase, false );
            menuOption->SetParentMenu( m_parent, this, m_options[index]->m_value );
            menuOption->SetRenderLanguage( option->m_renderLanguage );
