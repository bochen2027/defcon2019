...
:	EclButton(),
    ...
    m_inputNoExtendedCharacters(false)
{
	m_buf[0] = '\0';
}
...
void InputField::Keypress ( int keyCode, bool shift, bool ctrl, bool alt, unsigned char ascii )
{
    ...
    {
        if( m_type == TypeString && m_inputNoExtendedCharacters && ascii > 127 )
        {
            return;
        }
...
void InputField::SetInputNoExtendedCharacters( bool _inputNoExtendedCharacters )
{
	if( m_inputNoExtendedCharacters != _inputNoExtendedCharacters )
	{
		m_inputNoExtendedCharacters = _inputNoExtendedCharacters;
		if( m_inputNoExtendedCharacters && m_type == TypeString && m_string )
		{
			StripExtendedCharacters( m_string );
		}
	}
}
