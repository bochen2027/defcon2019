...
void StyleEditorWindow::Create()
{
    ...
    styleDefault->SetProperties( "Restore Default", m_w-490, m_h-30, 180, 20, "dialog_restore_default", " ", true, false );
    RegisterButton( styleDefault );

    LoadStylesButton *load = new LoadStylesButton();
    load->SetProperties( "Load", m_w-300, m_h-30, 140, 20, "dialog_load", " ", true, false );
    RegisterButton( load );

    SaveStylesButton *save = new SaveStylesButton();
    save->SetProperties( "Save", m_w-150, m_h-30, 140, 20, "dialog_save", " ", true, false );
    RegisterButton( save );
}
