...
void DspBitCrusher::Process( signed short *_data, unsigned int _numSamples)
{
    if( m_bitRate < 16.0f )
    {
        int valueRange = powf(2, m_bitRate);
        int stepSize = 65536 / valueRange;
...
DspSmoother::DspSmoother(int _sampleRate)
:   DspEffect(_sampleRate),
    m_smoothFactor(0)
{
}
...
void DspSmoother::SetParameters(float const *_params)
{
    m_smoothFactor = _params[0];
}


void DspSmoother::Process(signed short *_data, unsigned int _numSamples)
{
    // Assume stereo samples

    signed short prevL = 0;
    signed short prevR = 0;

    float bigDif = 32 * (100 - m_smoothFactor);

    for( int i = 0; i < _numSamples; i+=2 )
    {
        if( i != 0 )
        {
            signed short thisL = _data[i];
            signed short thisR = _data[i+1];

            float difL = thisL - prevL;
            if( difL > bigDif ) difL = bigDif;
            if( difL < -bigDif ) difL = -bigDif;
            thisL = prevL + difL;

            float difR = thisR - prevR;
            if( difR > bigDif ) difR = bigDif;
            if( difR < -bigDif ) difR = -bigDif;
            thisR = prevR + difR;
